...
won't work in Firefox or Safari at the moment. There is a [polyfill](https://github.com/flackr/scroll-timeline) to add
...
`IntersectionObserver`-API an therefore require client-side JavaScript to be downloaded for the animations to work. One
...
combination with the `view()` or `scroll()` helpers and its usage should bequite familiar to people who have worked with
CSS animations before.
...
a user navigates to a new page the browser just serves the new document. This is great for page performance, because
...
Only Chromium-based browsers support this API at the moment, however, Astro come with a polyfill to make the transitions
